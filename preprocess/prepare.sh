...
# adding tags
## train data
cat $BPE_DATA/train.bpe.${lang} | awk 'NR>=1 && NR <=133317 {print "<2vi> " $0}' > cat $BPE_DATA/train.bpe.${lang}.1
cat $BPE_DATA/train.bpe.${lang}.1 | awk 'NR>=133318 && NR <=266634 {print "<2en> " $0}' > cat $BPE_DATA/train.bpe.${lang}.2
cat $BPE_DATA/train.bpe.${lang}.2 | awk 'NR>=266635 && NR <=489742 {print "<2ja> " $0}' > cat $BPE_DATA/train.bpe.${lang}.3
cat $BPE_DATA/train.bpe.${lang}.3 | awk 'NR>=489743 && NR <=712850 {print "<2en> " $0}' > cat $TAGGED_DATA/train.${lang}

## validation
cat $BPE_DATA/valid.bpe.${lang} | awk 'NR>=1 && NR <=1553 {print "<2vi> " $0}' > cat $BPE_DATA/valid.bpe.${lang}.1
cat $BPE_DATA/valid.bpe.${lang}.1 | awk 'NR>=1554 && NR <=3106 {print "<2en> " $0}' > cat $BPE_DATA/valid.bpe.${lang}.2
cat $BPE_DATA/valid.bpe.${lang}.2 | awk 'NR>=3107 && NR <=3977 {print "<2ja> " $0}' > cat $BPE_DATA/valid.bpe.${lang}.3
cat $BPE_DATA/valid.bpe.${lang}.3 | awk 'NR>=3978 && NR <=4848 {print "<2en> " $0}' > cat $TAGGED_DATA/valid.${lang}

## test
cat $BPE_DATA/test.bpe.${lang} | awk 'NR>=1 && NR <=1268 {print "<2vi> " $0}' > cat $BPE_DATA/test.bpe.${lang}.1
cat $BPE_DATA/test.bpe.${lang}.1 | awk 'NR>=1269 && NR <=2536 {print "<2en> " $0}' > cat $BPE_DATA/test.bpe.${lang}.2
cat $BPE_DATA/test.bpe.${lang}.2 | awk 'NR>=2537 && NR <=3730 {print "<2ja> " $0}' > cat $BPE_DATA/test.bpe.${lang}.3
cat $BPE_DATA/test.bpe.${lang}.3 | awk 'NR>=3731 && NR <=4924 {print "<2en> " $0}' > cat $TAGGED_DATA/test.${lang}
